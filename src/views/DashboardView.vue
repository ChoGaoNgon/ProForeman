...
const stats = computed(() => {
  ...
  return {
    total: all.length,
    risk: all.filter(p => p.status_evaluation === 'RISK').length,
    warning: all.filter(p => p.status_evaluation === 'WARNING').length,
    safe: all.filter(p => p.status_evaluation === 'SAFE').length,
  };
});

const urgentProjects = computed(() => {
  return appStore.visibleProjects.filter(p => p.status_evaluation !== 'SAFE');
});
...
<template>
  ...
  <div v-else class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      ...
      <div v-if="urgentProjects.length > 0" class="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 rounded-full text-sm font-medium border border-red-100">
        <Activity :size="16" />
    ...
    <!-- Alert Section -->
    <div v-if="urgentProjects.length > 0" class="bg-white rounded-2xl border border-red-100 overflow-hidden shadow-sm">
      ...
      <div class="divide-y divide-neutral-100 overflow-x-auto">
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-neutral-100">
            <tr v-for="project in urgentProjects" :key="project.id" class="group hover:bg-neutral-50/50 transition-colors">
              <td class="px-6 py-4">
                ...
                <div class="space-y-3 max-w-[200px]">
                  <div>
                    <div class="flex justify-between text-[10px] font-bold mb-1">
                      ...
                      <span class="text-blue-600">25% HT</span>
                    </div>
                  ...
                  <div>
                    <div class="flex justify-between text-[10px] font-bold mb-1">
                      ...
                      <span class="text-emerald-600">28% TH</span>
                    </div>
                    <div class="w-full h-1.5 bg-emerald-100 rounded-full overflow-hidden">
                      <div class="h-full bg-emerald-600 rounded-full" style="width: 28%"></div>
                    </div>
                  </div>
                </div>
              </td>
